                        ...
                        <form class="space-y-4" method="POST" action="{{ route('login') }}">
                            ...
                            <div class="flex justify-between">
                                ...
                                @if (Route::has('password.request'))
                                    <x-link-text tabindex="-1" text="Forgot password?"
                                        href="{{ route('password.request') }}"></x-link-text>
                                @endif
                            </div>
                            <x-button-primary text="{{ __('Login') }}" type="submit"
                                class="w-full"></x-button-primary>

                            <div class="text-sm font-medium text-gray-500 dark:text-gray-300">
                                Dont have an account? <x-link-text text="Create an account"
                                    href="{{ route('register') }}"></x-link-text>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="p-4 md:p-5">
                        ...
                        <form method="POST" action="{{ route('register') }}">
                            ...
                            <div>
                                ...
                                <x-input-text id="email" type="email"
                            ...
                            <div>
                                ...
                                <x-input-text id="password" type="password" placeholder=""
                                    class="form-control @error('password') is-invalid @enderror" name="password"
                                    required autocomplete="new-password"></x-input-text>
                            </div>

                            <div>
                                <x-input-label for="password-confirm" text="Confirm Passsword" />
                                <x-input-text id="password-confirm" type="password" placeholder=""
                                    class="form-control" name="password_confirmation" required
                                    autocomplete="new-password"></x-input-text>
                            </div>
                            <div class="flex items-start mb-5">
                                ...
                                <label for="terms" class="ms-2 text-sm font-medium text-gray-300 ">I agree with the
                                    <a href="#" class="text-blue-600 hover:underline ">terms and
                                        conditions</a></label>
                            </div>
                            <x-button-primary text="Register" type="submit" class="w-full"></x-button-primary>
                            <div class="text-sm font-medium mt-2 text-gray-500 ">
                                Already have an account? <x-link-text text="Sign in"
                                    href="{{ route('login') }}"></x-link-text>
                            </div>
                        </form>
                    </div>
                </div>
